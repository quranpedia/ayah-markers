# Licences of the source outlines

**This file documents the terms of the fonts the outlines were traced from. It is not a licence for this repository.** The repository itself carries no `LICENSE` file and this document grants nothing on its behalf; it records what the *sources* require of anyone who redistributes the outlines.

Every outline in `markers/` is traced from a glyph of one of the families below. Each source is also recorded machine-readably in `collection.json`, under `markers[].sources[].license`.

## What the OFL asks of you
...
Copyright lines are quoted verbatim from each family's `OFL.txt`.
...
Twelve families were taken from `fonts.quran.ws`. **Their terms are not recorded anywhere in this repository and have not been confirmed with the publisher.** They are marked `unverified — pending confirmation` in `collection.json`.

This is deliberate. Asserting a licence we have not read would be worse than leaving the gap visible. In particular, DigitalKhatt is published under its own upstream terms, which are not the OFL. **Do not redistribute the markers below until their terms are confirmed.**
...
The text below is quoted from `ofl/notosansarabic/OFL.txt` in [google/fonts](https://github.com/google/fonts); it is the same OFL 1.1 text all fourteen families carry. Each family's own copyright line is in the table above and is the notice that must accompany that family's outlines.
